...
*/            },
    ...
    async queryMobileNumberIsExist(ctx, mobileNumbers,optedService) {

        console.log(" ************ in fabcar.js of queryMobileNumberIsExist ************ ")
        if(optedService===2)
        {
            console.log(" optedService check  ##false## ");
            return false;
        }
        const carAsBytes = await ctx.stub.getState(mobileNumbers); // get the car from chaincode state
        if (carAsBytes || carAsBytes.length != 0) {
            //throw new Error(`${mobileNumbers} already exist`);
    ...
    async createMobilenumber(ctx, mobileNumbers, serviceprovider, servicearea, preference, status, activationdate) {
